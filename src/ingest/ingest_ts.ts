...
export function ingestTypeScriptFiles(files: string[], root: string): Graph {
  ...
  for (const f of files) {
    try {
      ...
      if (!text) {
        errors.push({ file: f, error: 'File could not be read' });
        continue;
      }
      ...
      function visit(node: ts.Node) {
        try {
          ...
          errors.push({ file: f, error: `Error processing node: ${errorMsg}` });
        }
      }
      visit(sf);
  ...
  // Second pass: imports + calls (only for successfully processed files)
  for (const f of processedFiles) {
    try {
      ...
      function visit(node: ts.Node) {
        try {
          if (!moduleSym) return; // Skip if module symbol not found
          ...
          // Continue processing other nodes even if one fails
          const errorMsg = err instanceof Error ? err.message : String(err);
          errors.push({ file: f, error: `Error processing edge in node: ${errorMsg}` });
        }
      }
      visit(sf);
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : String(err);
      errors.push({ file: f, error: `Error in second pass: ${errorMsg}` });
    }
  }

  // Report errors if any
  if (errors.length > 0) {
    console.error(`[ingest] TypeScript ingestion completed with ${errors.length} error(s):`);
